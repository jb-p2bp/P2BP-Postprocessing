...
* `merged-point-cloud.top-down-view.png` (floor-aligned outdoor site-plan rendering)
...
and a JSON registration report. The queue worker subsequently renders the merged
LAZ as a floor-aligned outdoor site-plan PNG. Source packages are never modified; the
first input scan anchors the output frame.
